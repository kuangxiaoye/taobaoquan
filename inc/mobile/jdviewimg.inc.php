<?php
	//小程序 数据列表
	//http://cs.tiger-app.com/app/index.php?i=8&c=entry&a=wxapp&do=jdviewimg&m=tiger_newhu&itemid=27221768725
global $_W, $_GPC;		
		$itemid=$_GPC['itemid'];
		$url="https://item.jd.com/".$itemid.".html";
		$str=curl_request($url);		
		$aa=Text_qzj($str,"desc: '","',");//获取详情链接
		$aa="http:".$aa;
		echo $aa;
		$str=curl_request($aa);		
		$desc=Text_qzj($str,'"content":"','"}');//获取详情链接
		$desc=str_replace("data-lazyload=","src=",$desc);
		$img=str_replace('\"','"',$desc);
//		echo $desc;
