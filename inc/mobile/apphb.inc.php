...
$weid=$_W['uniacid'];
        ...
        //海报背景
        $hbbj = pdo_fetch("select * from ".tablename("tiger_app_hb")." where weid='{$weid}'");
				//print_r($hbbj);

        //$url1=$xcxcfg['zdyurl']."u.php?u=".$uid."&i=".$_W['uniacid']."&yq=".$share['yaoqingma'];
				$url1=$cfg['tknewurl']."app/index.php?i=".$weid."&c=entry&do=apphbfx&m=tiger_newhu&yq=".$share['yaoqingma'];
			 $url=urlencode($url1);

        $ewmp="http://bshare.optimix.asia/barCode?site=weixin&url=".$url;
        
        $ewm=appgetimg($ewmp,'',$_W);
        $hbimg=picjialidun($ewm,$hbbj['pic'],$cfg['tknewurl'],$share);
        //小图
	   ...
	   **/
	  	function picjialidun($ewm,$pic,$httpsurl,$share){//生成产品海报图片
	        ...
	        if(empty($pic)){
	        	...
	        	$background_pic_path=IA_ROOT.'/attachment/'.$pic;
	        }
	        ...
	        imagettftext($im, 26, 0, 206,930, $color, $font,$share['yaoqingma']);
	        ...
	        $aa=imagecopyresampled($im, $source, 195,700, 0, 0, $newwidth, $newheight, $width, $height);//二维码位置
