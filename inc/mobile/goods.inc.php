...
//        exit;
        ...
        $xn_list = pdo_fetchall("SELECT * FROM " . tablename($this -> table_goods) . " WHERE weid = '{$_W['weid']}' and $now < endtime and amount >= 0 and type=5 order by px ASC");
        $sw_list = pdo_fetchall("SELECT * FROM " . tablename($this -> table_goods) . " WHERE weid = '{$_W['weid']}' and $now < endtime and amount >= 0 and type<>5 order by px ASC");

        


        $my_goods_list = pdo_fetch("SELECT * FROM " . tablename($this -> table_request) . " WHERE  from_user='{$fans['openid']}' AND weid = '{$_W['weid']}'");
