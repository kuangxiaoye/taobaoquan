...
//          exit;
            ...
            if(!empty($import)){
                foreach($import as $k=>$v){               
                   ...
                   $ord=pdo_fetch ( 'select orderid from ' . tablename ( $this->modulename . "_tkorder" ) . " where weid='{$_W['uniacid']}' and numid='{$v[3]}'  and orderid='{$v[25]}'" );
                   if(empty($ord)){
                      if(!empty($data['addtime'])){
                         pdo_insert ($this->modulename . "_tkorder", $data );
                      } 
                   }else{
                     pdo_update($this->modulename . "_tkorder",$data, array ('orderid' =>$data['orderid'],'numid'=>$data['numid'],'weid'=>$_W['uniacid']));
                   }
